import EmotionRRegistry from "@/lib/emotion";
import { inter } from "./fonts";
import "./globals.css";

export const metadata = {
  title: "Create Next App",
  description: "Generated by create next app",
};

const showPrototype = true;

const Content = ({ children }: { children: JSX.Element }) => (
  <>
    <div className="absolute top-xs w-full">
      <div
        className="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md"
        role="alert"
      >
        <div className="flex">
          <div className="py-1">
            <svg
              className="fill-current h-6 w-6 text-teal-500 mr-4"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
              <path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM9 11V9h2v6H9v-4zm0-6h2v2H9V5z" />
            </svg>
          </div>
          <div>
            <p className="font-bold">Our privacy policy has changed</p>
            <p className="text-sm">
              Make sure you know how these changes affect you.
            </p>
          </div>
        </div>
      </div>
    </div>
    <EmotionRRegistry>{children}</EmotionRRegistry>
  </>
);

const MobileCover = ({ children }: { children: JSX.Element }) => (
  <html lang="en" className={inter.className} data-theme="mytheme">
    {/* <body className="h-screen bg-gradient-to-r from-[#2e026d] to-[#15162c]"> */}
    <body className="h-screen flex items-center text-sm">
      <div className="mockup-phone">
        <div className="camera"></div>
        <div className="display">
          <div className="artboard artboard-demo phone-1 h-screen bg-gradient-to-r from-[#2e026d] to-[#15162c] overflow-auto relative">
            <Content>{children}</Content>
          </div>
        </div>
      </div>
    </body>
  </html>
);

const Page = ({ children }: { children: JSX.Element }) => (
  <html lang="en" className={inter.className} data-theme="mytheme">
    <body className="h-screen bg-gradient-to-r from-[#2e026d] to-[#15162c]">
      <Content>{children}</Content>
    </body>
  </html>
);

export default function RootLayout({ children }: { children: JSX.Element }) {
  return (!showPrototype ? Page : MobileCover)({ children });
}
